...
type HistorialPageProps = {
  searchParams: Promise<{ plate?: string | string[]; finished?: string; reopened?: string }>
}
...
export default async function HistorialPage({ searchParams }: HistorialPageProps) {
  ...
  let query = supabase
    .from('trips')
    .select('id, plate, driver, product, warehouse, destination, loading_date, finished_at')
    .eq('status', 'FINISHED')
  ...
  return (
    <AppShell>
      ...
      {params.finished === '1' ? <p role="status" className="mt-6 rounded-lg bg-green-50 px-4 py-3 text-sm text-green-700">El viaje fue finalizado y se movió al historial.</p> : null}
      {params.reopened === '1' ? <p role="status" className="mt-6 rounded-lg bg-green-50 px-4 py-3 text-sm text-green-700">El viaje fue reabierto y volvió a los viajes activos.</p> : null}
      ...
      {error ? (
        <section role="alert" className="mt-6 rounded-xl bg-red-50 p-5 text-sm text-red-700">No fue posible cargar el historial. Inténtalo de nuevo.</section>
      ) : trips?.length ? (
        <>
          <div className="mt-6 hidden overflow-x-auto rounded-xl border border-slate-200 bg-white lg:block">
